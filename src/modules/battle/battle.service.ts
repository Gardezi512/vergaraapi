...
async create(dto: CreateBattleDto, user: User): Promise<Battle> {
  ...
  const battle = this.battleRepo.create({
    ...
    createdBy: user,
    isByeBattle: isBye,
    status: isBye ? BattleStatus.COMPLETED : BattleStatus.PENDING,
...
// }
// 🎯 NEW: Add ELO calculation method (client's exact formula)
private async updateThumbnailELO(winnerThumb: Thumbnail, loserThumb: Thumbnail): Promise<void> {
  ...
  const expectedWinner = 1 / (1 + Math.pow(10, (loserThumb.eloRating - winnerThumb.eloRating) / 400));
  const expectedLoser = 1 / (1 + Math.pow(10, (winnerThumb.eloRating - loserThumb.eloRating) / 400));

  // Update ELO ratings
  const newWinnerELO = Math.round(winnerThumb.eloRating + kWinner * (1 - expectedWinner));
  const newLoserELO = Math.round(loserThumb.eloRating + kLoser * (0 - expectedLoser));
...
async resolveWinnerFromVotes(battleId: number): Promise<any> {
  ...
  const battle = await this.battleRepo.findOne({
    where: { id: battleId },
    relations: ["thumbnailA", "thumbnailB", "thumbnailA.creator", "thumbnailB.creator", "tournament"],
  })
  ...
  if (votesA > votesB) {
    ...
    this.logger.log(`Tie in battle ${battle.id}. Using ELO-based tie-breaker.`)
...
async generateRandomBattlesForRound(tournamentId: number, roundNumber: number, createdBy: User): Promise<Battle[]> {
  ...
  for (let i = 0; i < shuffled.length; i += 2) {
    ...
    if (!thumbnailA || !thumbnailB) {
      this.logger.error("❌ Invalid pairing: one or both thumbnails are null.");
      continue;
    }
  ...
  for (const battle of battles) {
    ...
    if (battle.isByeBattle) {
      if (!battle.thumbnailA?.creator) {
        this.logger.error(`Bye battle ${battle.id} has no creator on thumbnailA. Skipping.`);
        continue;
      }
      ...
      if (!winningThumb.creator) {
        this.logger.error(`Winning thumbnail ${winningThumb.id} has no creator. Skipping.`);
        continue;
      }
    }
